...
def create_table():
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS list_bola (id INTEGER not null PRIMARY KEY, nama_penyewa TEXT not null, jumlah_bola INTEGER not null, harga_total INTEGER not null)")
    conn.commit()
    conn.close()


def insert(id, nama_penyewa, jumlah_bola, harga_total):
    conn = sqlite3.connect("futsal_ku.db")
    cursor = conn.cursor()
    cursor.execute("INSERT INTO list_bola (id, nama_penyewa, jumlah_bola, harga_total) VALUES(?,?,?,?)", (id, nama_penyewa, jumlah_bola, harga_total))
    conn.commit()
    conn.close()
...
def select_all():
    conn = sqlite3.connect("futsal_ku.db")
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM list_bola")
...
def update_jumlahBola_on_id(id, jumlah_bola):
    conn = sqlite3.connect("futsal_ku.db")
    cursor = conn.cursor()
    cursor.execute(f"UPDATE list_bola SET jumlah_bola={jumlah_bola} WHERE id={id}")
    conn.commit()
    conn.close()
